        ...
        if len(layer_activation.shape) == 4:  # Convolutional layer (batch_size, height, width, num_filters)
            ...
            if layer_name == 'dense':#the last layer before uotput
                with st.expander("Click here to see inside my neurons:"):
                    ...
                    st.write("In this plot, you finally see, what is inside the key part of my neural network. From the neurons, you can’t directly identify which parts of the image are most applish in terms of raw visual features because neurons don’t correspond to specific image areas. Instead, each neuron is activated by combinations of features that may span across the entire image, contributing my understanding of what makes something apple-like or pear-like.However, I can highlight the neurons that are most influential for the apple prediction relative to the pear prediction. These are the neurons whose weighted activations have the strongest effect on the final prediction of apple, compared to pear. This can be determined by identifying which neurons have higher weighted activations for apple, even if both apple and pear activations are negative. Essentially, we are searching for what makes the image more apple-like by looking at which neurons contribute more towards apple than pear.")